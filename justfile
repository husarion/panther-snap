...
dev-install:
    #!/bin/bash
    if ls panther-ros_*.snap 1> /dev/null 2>&1; then
        sudo snap remove --purge panther-ros
        sudo snap install ./panther-ros_*.snap --devmode; \
    else \
...
dev-launch:
    #!/bin/bash
    sudo snap connect panther-ros:gpio-control
    sudo snap connect panther-ros:spi pi:spidev0
    sudo snap connect panther-ros:spi pi:spidev1

stop:
    sudo snap disconnect panther-ros

info:
    sudo snap get panther-ros
    sudo snap connections panther-ros

logs:
    sudo snap logs panther-ros -n 20
